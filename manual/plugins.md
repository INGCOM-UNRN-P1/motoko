---
title: "Guía de Extensión y Creación de Plugins: motoko"
subtitle: "Manual de integración, desarrollo de extensiones y uso de la API Python de motoko"
author: "Cátedra de Algoritmos y Programación"
date: "2026-08-31"
---

(manual-motoko-plugins)=
# Guía de Extensión y Plugins: motoko

````{abstract}
Esta guía técnica detalla cómo desarrollar extensiones, crear nuevos plugins e integrar programáticamente **`motoko`** en herramientas de evaluación, entornos de integración continua (CI/CD) o scripts docentes.
````

---

(manual-motoko-plugins-arquitectura)=
## 1. Arquitectura de Extensión

`motoko` provee una arquitectura modular desacoplada basada en puntos de entrada (Entry Points) estándar de Python (`[project.entry-points]`) o interfaces de inyección funcional:

- **Mecanismo de Extensión Principal**: `Extensiones y Reglas Personalizadas para motoko`.
- **Punto de Entrada Oficial**: `motoko.plugins`.
- **Formato de Comunicación**: Estructuras de datos serializables JSON / Pydantic models.

---

(manual-motoko-plugins-tutorial)=
...
class CustomPlugin:
    """Plugin de extensión para motoko."""
    name = "custom_rule"
...
Para que `motoko` descubra y cargue automáticamente tu plugin, agregalo en tu `pyproject.toml`:

````{code-block} toml
[project.entry-points."motoko.plugins"]
mi_plugin = "mi_paquete.modulo:MiPlugin"
````

### Paso 3: Instalar y Verificar el Plugin

Instalá tu extensión en modo editable y comprobá que `motoko` la reconozca:
...
(manual-motoko-plugins-sdk)=
## 3. Conexión Programática mediante la API Python

Podés importar y ejecutar `motoko` directamente desde scripts de Python sin invocar subprocesos:
...
resultado = motoko.ejecutar_analisis(
...
(manual-motoko-plugins-ci)=
## 4. Integración en Pipelines de CI/CD (GitHub Actions / GitLab CI)

Podés integrar `motoko` en tus flujos automatizados de Git para bloquear entregas que no cumplan los requisitos de cátedra:
...
jobs:
  auditoria:
    ...
    steps:
      ...
      - name: Instalar motoko
        run: pip install -e ./motoko
        
      - name: Ejecutar Auditoría
        run: motoko check src/ include/ --json > reporte.json
````

---

(manual-motoko-plugins-ejercicios)=
